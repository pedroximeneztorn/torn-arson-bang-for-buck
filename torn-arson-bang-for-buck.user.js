...
(function () {
  ...
  async function getPricesFromAPI() {
    ...
    try {
      ...
      if (Object.keys(updatedValues).length > 0) {
        ...
        saveItemValues();
        console.log(
          "[ArsonBangForBuck] Updated item values from API:",
          updatedValues,
        );
        return true; //  Success
      } else {
  ...
  const scenarios = {
    ...
    "Burning Ambition": [
      ["Profit/Nerve: ", "Flamethrower: No ", "Place: ", "Stoke: ", "Dampen: "],
      [
        "Payout: 46K",
        "Profit/Nerve: 2.7K",
        "Flamethrower: Yes",
    ...
    lines.forEach((line) => {
      ...
      if (/^(Place|Stoke|Dampen|Evidence)/i.test(line)) {
        ...
        while ((m = regex.exec(cleaned)) !== null) {
          ...
          const item = m[2].trim();
          const key = Object.keys(itemValues).find(
  ...
  function loadHighlightValues() {
    ...
    if (saved) {
      try {
        highlightValues = JSON.parse(saved);
      } catch (e) {
        console.error("Failed to parse saved highlight values:", e);
      }
    }
  }
  function saveHighlightValues() {
    localStorage.setItem("highlightValues", JSON.stringify(highlightValues));
  }
  loadHighlightValues();

  //  Helpers for cost/profit
  function parseValue(value) {
    return value.toUpperCase().endsWith("K")
  ...
  function createSettingsUI() {
    ...
    const header = document.querySelector(
      appHeaderClass
        ? `.${appHeaderClass}.crimes-app-header`
        : ".crimes-app-header",
    );
    if (!header) return;
    ...
    if (hasArson) {
      if (!existingButton) {
        ...
        helpLink.href =
          "https://www.torn.com/forums.php#/p=threads&f=67&t=16518811&b=0&a=0";
        ...
        function renderHighlightValues() {
          ...
          ["LowProfit", "HighProfit"].forEach((key) => {
            ...
            input.onchange = () => {
              highlightValues[key] = parseInt(input.value, 10);
              saveHighlightValues();
            };
            row.appendChild(label);
            row.appendChild(input);
        ...
        function updateActionButton() {
          const existingActionBtn = document.querySelector(
            "#settingsPanel button.action-btn",
          );
          ...
          if (
            contentDiv.innerHTML.includes("Fuel Items") ||
            ...
            actionButton.onclick = async () => {
              if (
                confirm(
                  "Update item values from Torn API? This will overwrite your current settings.",
                )
              ) {
                ...
                if (success) {
                  ...
                  alert(
                    "Failed to update item values. Check your API key or Torn API status.",
                  );
                }
              }
            };
        ...
        newButton.addEventListener("click", () => {
          newPanel.style.display =
            newPanel.style.display === "none" || newPanel.style.display === ""
              ? "block"
              : "none";
        });
  ...
  function createTooltip(lines, section, highlightTarget) {
    ...
    if (ranges) {
      ...
      const baseProfitValue = ranges.profitText
        .replace(/<[^>]*>/g, "") // remove HTML tags
        .split("–")
        .pop()
        .trim(); // take the base value (right side)
      const numericValue =
        parseFloat(baseProfitValue.replace(/K/i, "")) *
        (baseProfitValue.includes("K") ? 1000 : 1);
  ...
  function showTooltip(tooltip, target) {
    const visibleTooltip = document.querySelector(
      '.custom-tooltip[style*="display: flex"]',
    );
  ...
  function shouldShowScenario(lines, hasFlamethrower) {
    const flamethrowerLine = lines.find((line) =>
      line.trim().toLowerCase().startsWith("flamethrower:"),
    );
    if (!flamethrowerLine) return true;
    if (hasFlamethrower && flamethrowerLine.toLowerCase().includes("no"))
      return false;
    if (!hasFlamethrower && flamethrowerLine.toLowerCase().includes("yes"))
      return false;
    return true;
  }
  ...
  function addTooltips() {
    ...
    document.querySelectorAll(`.${sectionsClass}`).forEach((section) => {
      ...
      const selectedVariant = Array.isArray(variants[0])
        ...
        : shouldShowScenario(variants, hasFlamethrower)
          ? variants
          : null;
      ...
      const hoverTarget = hoverSelector
        ? section.querySelector(hoverSelector)
        : null;
      const iconTarget = titleClass
        ? section.querySelector(`.${titleClass}`)
        : null; // ✅ Only this small clickable area

      // Desktop hover
      if (hoverTarget) {
        hoverTarget.addEventListener("mouseenter", () =>
          showTooltip(tooltip, hoverTarget),
        );
